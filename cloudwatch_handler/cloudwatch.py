...
class CloudwatchHandler(logging.Handler):
    ...
    def __init__(
        ...
        if not log_stream:
            ...
            log_stream = "{}{}".format(
                datetime.strftime(datetime.now(), "%Y%m%d%H%M%S%f"), randrange(100)
            )
        # Set them in self
        ...
        if not access_id and not region:
            ...
            self.session = self.boto3.Session(
                aws_access_key_id=access_id, aws_secret_access_key=access_key, region_name=region
            )
        ...
        try:
            ...
            for l in response["logStreams"]:
                if l["logStreamName"] == self.log_stream:
                    self.next_sequence_token = (
                        l["uploadSequenceToken"] if "uploadSequenceToken" in l else None
                    )
            ...
            if self.next_sequence_token == None:
                ...
                self.logs.put_retention_policy(
                    logGroupName=self.log_group, retentionInDays=retention_period
                )
    ...
    def emit(self, record):
        ...
        try:
            ...
            exception_text = str(e)
            self.next_sequence_token = exception_text[
                exception_text.find("sequenceToken is: ") + 18 :
            ]
            self.send_log(timestamp, log_entry)
